...
class UpShuffle2D(torch.nn.Module):
    def __init__(self, channels, scale=4, num_out_ch=3):
        super().__init__()
        self.conv = torch.nn.Conv2d(channels, num_out_ch * scale * scale, kernel_size=3, stride=1, padding=1)
...
class SRNext(nn.Module):
    def __init__(self, embed_dim=60, num_in_ch=3, num_out_ch=3, blocks=4, num_active=4):
        ...
        self.out = UpShuffle2D(embed_dim, num_out_ch)
